...
//设置界面显示的图层
- (void)setInterface:(UIView *)iView{
    
    CGFloat width = iView.bounds.size.width/5;
    CGFloat height = iView.bounds.size.height/3;
    UIColor *color = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.5];
//    UIColor *color1 = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.5];

//    CALayer *topLayer = [CALayer layer];
//    topLayer.position = CGPointMake(iView.bounds.size.width/2, height/2);
//    topLayer.bounds = CGRectMake(0, 0, iView.bounds.size.width, height);
//    topLayer.backgroundColor = color1.CGColor;
//    [iView.layer addSublayer:topLayer];
    UIView *topView = [UIView new];
    topView.frame = CGRectMake(0, 0, width*5, height);
    topView.backgroundColor = color;
    [iView addSubview:topView];
    
    UIButton * flashBtn=[UIButton buttonWithType:UIButtonTypeCustom];
    flashBtn.frame = CGRectMake(topView.frame.size.width-55,80, 35, 49);
    [flashBtn setBackgroundImage:[UIImage imageNamed:@"shan"] forState:UIControlStateNormal];
    ...
    leftView.frame = CGRectMake(0, height, width, height);
    leftView.backgroundColor = color;
    [iView addSubview:leftView];
    
    UIView *bottomView = [UIView new];
    bottomView.frame = CGRectMake(0, height*2, width*5, height);
    bottomView.backgroundColor = color;
    [iView addSubview:bottomView];
    
    UILabel * tipLabel = [[UILabel alloc] initWithFrame:CGRectMake(bottomView.bounds.size.width/2-50, 10, 100, 30)];
    ...
    rightView.frame = CGRectMake(width*4, height, width, height);
    rightView.backgroundColor = color;
    [iView addSubview:rightView];
    
//    CALayer *leftLayer = [CALayer layer];
//    leftLayer.position = CGPointMake(width/2, height + height/2);
//    leftLayer.bounds = CGRectMake(0, 0, width, height);
//    leftLayer.backgroundColor = color1.CGColor;
//    [iView.layer addSublayer:leftLayer];
//
//    CALayer *bottomLayer = [CALayer layer];
//    bottomLayer.position = CGPointMake(iView.bounds.size.width/2, 2*height + height/2);
//    bottomLayer.bounds = CGRectMake(0, 0, iView.bounds.size.width, height);
//    bottomLayer.backgroundColor = color1.CGColor;
//    [iView.layer addSublayer:bottomLayer];
//    
//    CALayer *rightLayer = [CALayer layer];
//    rightLayer.position = CGPointMake(width*4 + width/2, height + height/2);
//    rightLayer.bounds = CGRectMake(0, 0, width, height);
//    rightLayer.backgroundColor = color1.CGColor;
//    [iView.layer addSublayer:rightLayer];
...
-(void)setCenterLayerView
{
    CGFloat width = self.view.bounds.size.width/5;
    CGFloat height = self.view.bounds.size.height/3;
    UIColor *color = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.1];
    UIView *centerView = [[UIView alloc]initWithFrame:CGRectMake(width, height, width*3, height)];
    centerView.backgroundColor = color;
    centerView.clipsToBounds = YES;
    imageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"1"]];
    imageView.frame = CGRectMake(5, -centerView.bounds.size.height, centerView.bounds.size.width, centerView.bounds.size.height);
    imageView.layer.position = CGPointMake(centerView.bounds.size.width/2, -centerView.bounds.size.height/2);
    for (int i = 0; i < 4; i ++) {
        [self drawRightAngle:centerView tag:i];
    }
    
    NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:3 target:self selector:@selector(upTimer) userInfo:nil repeats:YES];
    [timer fire];
    
    [self.view addSubview:centerView ];
    [centerView insertSubview:imageView atIndex:0];
}

//扫描框的小直角
- (void)drawRightAngle:(UIView *)iView tag:(int)tag{
    ...
    switch (tag) {
        ...
        case 1:
        {
            CGAffineTransform transform = CGAffineTransformMakeTranslation(iView.bounds.size.width, 0);
            shapeLayer.affineTransform = CGAffineTransformRotate(transform, M_PI/2);
        }
            break;
        case 2:
        {
            CGAffineTransform transform = CGAffineTransformMakeTranslation(iView.bounds.size.width, iView.bounds.size.height);
            shapeLayer.affineTransform = CGAffineTransformRotate(transform, M_PI);
        }
            break;
        case 3:
        {
            CGAffineTransform transform = CGAffineTransformMakeTranslation(0, iView.bounds.size.height);
...
-(void)upTimer
{
    ...
    animation.toValue = [NSValue valueWithCGPoint:CGPointMake(position.x, position.y+10)];
...
//    _newShadow.bounds = CGRectMake(0, 0, layer.bounds.size.width, 5);
//    //添加渐变的颜色组合（颜色透明度的改变）
